        ...
        <table class="table table-bordered align-middle">
            ...
            %>
                ...
                <tr>
                    <td><%=stt%></td>
                    <td>
                        <img src="<%=url%>/Products/<%=(p.getProductImages().isEmpty())?"":p.getProductImages().get(0).getUrl()%>"  class="img-fluid rounded" style="max-width: 80px;">
                        <span class="ms-2"><%=p.getNameProduct()%></span>
                    </td>
                    <td><%=p.getSizePrices().get(0).getDiameter()%></td>
<%--                    <td><%=nF.format(p.getPrice())%></td>--%>
                    <td><%=p.getQuantity()%>/td>
                        
<%--                    <td class="subtotal"><%=nF.format(p.getPrice()*p.getQuantity())%></td>--%>
                </tr>
                <%
